...
> {
  ...
  function scrollToBottom() {
    ...
    const opts = getOptions()
    // pageMode: the scroller's own `el` has no overflow; scroll the
    // resolved scroll-parent (custom element or window) instead.
    const scrollEl = opts.pageMode
      ? (resolveScrollParent(el.value, toValue(opts.scrollParent)) ?? window)
      : el.value
    if (!scrollEl)
      return
    ...
    const getMaxScroll = () => {
      if (scrollEl === window) {
        ...
        return Math.max(de.scrollHeight, document.body?.scrollHeight ?? 0)
      }
      return (scrollEl as HTMLElement).scrollHeight
    }
    const setScrollTop = (v: number) => {
      if (scrollEl === window) {
        window.scrollTo({ top: v })
      } else {
        (scrollEl as HTMLElement).scrollTop = v
      }
    }

    nextTick(() => {
      setScrollTop(getMaxScroll() + 5000)
      const cb = () => {
        setScrollTop(getMaxScroll() + 5000)
        requestFrame(() => {
          setScrollTop(getMaxScroll() + 5000)
